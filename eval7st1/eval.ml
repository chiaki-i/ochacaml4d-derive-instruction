...
let rec idc = fun s t m -> match s with
    ...
    begin match t with
        ...
        begin match m with
            MNil -> v
          | MCons ((c, s, t), m) ->
            let app_c0 = fun (v :: s) t m -> app_s v c s t m in
            app_c0 (v :: s) t m
        end
      | Trail (h) -> h v TNil m
    end
...
and f e xs vs c s t m = match e with
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (fun (v1 :: s) t m ->
      f e0 xs vs (fun (v :: v1 :: s) t m ->
        app v v1 c s t m) (v1 :: s) t m) s t m
...
and f_s e2s xs vs c s t m = match e2s with
  ...
  | e :: e2s ->
    f_s e2s xs vs (fun s t m ->
      f e xs vs (fun (v :: s) t m ->
        c (v :: s) t m) s t m
      ) s t m

(* f_t : e -> string list -> v list -> c -> s -> t -> m -> v *)
and f_t e xs vs c s t m =
  let app_c = fun (v :: s) t m -> app_s v c s t m in
  match e with
  ...
  | Fun (x, e) ->
    begin match s with
      ...
      | _ -> failwith "f_t: Fun: stack is empty"
    end
  | App (e0, e2s) ->
    f_s e2s xs vs (fun s t m ->
      begin match s with v1 :: s ->
        f e0 xs vs (fun (v0 :: v1 :: s) t m ->
          app v0 v1 app_c s t m) (v1 :: s) t m
...
and f_sr e xs vs c s t m =
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (fun (v1 :: s) t m ->
      f e0 xs vs (fun (v :: v1 :: s) t m ->
        app v v1 app_c0 s t m) (v1 :: s) t m) s t m
