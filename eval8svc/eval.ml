...
let rec app_s v0 v2s vs c s r t m = match v2s with
    [] -> run_c8 c (v0 :: s) r t m
  | v1 :: v2s ->
    app v0 v1 (app >>> c)) (VArgs (v2s) :: s) (VS (vs) :: r) t m
...
let rec f e xs = match e with
    Num (n) -> num n >>> C0
  | Var (x) -> access (Env.off_set x xs) >>> C0
  | Op (e0, op, e1) -> (* f e1 xs >> f e0 xs >> operation (op) *)
    (f e1 xs) >> (f e0 xs) >> (operation (op) >>> C0)
  | Fun (x, e) -> (grab (f e (x :: xs)) >>> C0)
  | App (e0, e2s) ->
    (f_s e2s xs) >> (f e0 xs) >> (app >>> C0)
  | Shift (x, e) -> (shift (f e (x :: xs)) >>> C0)
  | Control (x, e) -> (control (f e (x :: xs)) >>> C0)
  | Shift0 (x, e) -> (shift0 (f e (x :: xs)) >>> C0)
  | Control0 (x, e) -> (control0 (f e (x :: xs)) >>> C0)
  | Reset (e) -> (reset (f e xs) >>> C0)

(* f_s : e list -> string list -> c *)
and f_s e2s xs = match e2s with
    [] -> (pushmark >>> C0)
  | e :: e2s -> (f_s e2s xs) >> (f e xs) >> (push >>> C0)

(* f_init : e -> v *)
let f_init expr = run_c8 (f expr []) (VS ([])) C0 [] [] TNil MNil
