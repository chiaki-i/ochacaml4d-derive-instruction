...
(* initial continuation : a -> s -> t -> m -> v *)
let idc a s t m = match s with [] ->
    begin match t with
        TNil ->
        begin match m with
            MNil -> a
          | MCons ((c, s, t), m) -> c a s t m
        end
      | Trail (h) -> h a TNil m
    end
...
let rec f e xs vs c a s t m =
  match e with
    Num (n) -> c (VNum (n)) s t m
  | Var (x) -> c (List.nth vs (Env.off_set x xs)) s t m
  | Op (e0, op, e1) ->
    f e1 xs vs (fun v1 s t m ->
      f e0 xs vs (fun v0 s t m ->
        begin match (v1, v0) with
            (VNum (n0), VNum (n1)) ->
            begin match op with
          ...
          | _ -> failwith (to_string v0 ^ " or " ^ to_string v1 ^ " are not numbers")
        end) v1 s t m) a s t m
  | Fun (x, e) ->
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (fun v s t m ->
      f e0 xs vs (fun v ((v1 :: v2s) :: s) t m ->
        app v v1 c (v2s :: s) t m) v s t m) a s t m
...
and f_t e xs vs c a s t m =
  ...
  | Fun (x, e) ->
    begin match s with
        [] :: s -> c (VFun (fun v1 c' s' t' m' ->
          f_t e (x :: xs) (v1 :: vs) c' v1 s' t' m')) s t m
      | (v1 :: v2s') :: s -> f_t e (x :: xs) (v1 :: vs) c v1 (v2s' :: s) t m
    end
  | App (e0, e2s) ->
    f_s e2s xs vs (fun v (v2s :: v2s' :: s) t m ->
      f e0 xs vs (fun v ((v1 :: v2s) :: s) t m ->
        app v v1 c (v2s :: s) t m)
        v ((v2s @ v2s') :: s) t m) a s t m
  ...
  | e :: e2s ->
    f_s e2s xs vs (fun v s t m ->
      f e xs vs (fun v (v2s :: s) t m ->
        c v ((v :: v2s) :: s) t m) v s t m) a s t m
