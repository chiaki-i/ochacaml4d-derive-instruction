...
(* defunctionalize eval9st: eval9st *)
...
let appterm i = fun vs c s t m ->
  i vs (fun (v :: VArgs (v2s) :: s) t m -> apply9s v v2s c s t m) s t m
...
let rec f9 e xs = match e with
  ...
  | Op (e0, op, e1) ->
    f9 e1 xs >> f9 e0 xs >> operation op
  | Fun (x, e) -> cur (f9 e (x :: xs))
  | App (e0, e2s) ->
...
and f9t e xs = match e with
  ...
  | Op (e0, op, e1) ->
    f9 e1 xs >> f9 e0 xs >> operation op >> apply
  | Fun (x, e) -> grab (f9 e (x :: xs))
  | App (e0, e2s) -> f9s e2s xs >> appterm (f9t e0 xs)
  | Shift (x, e) -> shift (f9 e (x :: xs)) >> apply
  | Control (x, e) -> control (f9 e (x :: xs)) >> apply
  | Shift0 (x, e) -> shift0 (f9 e (x :: xs)) >> apply
  | Control0 (x, e) -> control0 (f9 e (x :: xs)) >> apply
  | Reset (e) -> reset (f9 e xs) >> apply

(* f : e -> v *)
let f expr = f9 expr [] [] idc [] TNil MNil
