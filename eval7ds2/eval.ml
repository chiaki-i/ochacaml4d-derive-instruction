...
let rec run_c7 c s t m = match (c, s) with
  ...
  | (CApp0 (vs, c), v :: VArgs (v2s) :: s) -> apply7s v v2s vs c s t m
  | (CApp2 (e0, xs, vs, c), VArgs (v2s) :: s) ->
    f7 e0 xs vs (CApp0 (vs, c)) (VArgs (v2s) :: s) t m
  | (CAppS0 (vs, c), v :: VArgs (v2s) :: s) ->
    run_c7 c (VArgs (v :: v2s) :: s) t m
  | (CAppS1 (e, xs, vs, c), VArgs (v2s) :: s) ->
    f7 e xs vs (CAppS0 (vs, c)) (VArgs (v2s) :: s) t m
  | (COp0 (op, vs, c), v :: v0 :: s) ->
  ...
  | (COp1 (e0, xs, op, vs, c), v :: s) ->
    f7 e0 xs vs (COp0 (op, vs, c)) (v :: s) t m
  ...
  | Fun (x, e) ->
    begin match (c, s) with
      (CApp0 (vs', c'), VArgs (v1 :: v2s) :: s') -> (* Grab *)
             f7 e (x :: xs) (v1 :: vs) (CApp0 (vs', c')) (VArgs (v2s) :: s') t m
...
and f7s e2s xs vs c s t m = match e2s with
  ...
  | e :: e2s ->
    f7s e2s xs vs (CAppS1 (e, xs, vs, c)) s t m
...
and apply7s v0 v2s vs c s t m = match v2s with
    [] -> run_c7 c (v0 :: s) t m
  | v1 :: v2s -> apply7 v0 v1 (CApp0 (vs, c)) (VArgs (v2s) :: s) t m
