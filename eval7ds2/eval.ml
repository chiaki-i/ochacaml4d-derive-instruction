...
let rec run_c7 c s t m = match (c, s) with
  ...
  | (CApp0 (vs, c), v :: VArgs (v2s) :: s) -> app_s v v2s vs c s t m
  | (CApp2 (e0, xs, vs, c), VArgs (v2s) :: s) ->
    f e0 xs vs (CApp0 (vs, c)) (VArgs (v2s) :: s) t m
  | (CAppS0 (vs, c), v :: VArgs (v2s) :: s) ->
    run_c7 c (VArgs (v :: v2s) :: s) t m
  | (CAppS1 (e, xs, vs, c), VArgs (v2s) :: s) ->
    f e xs vs (CAppS0 (vs, c)) (VArgs (v2s) :: s) t m
  | (COp0 (op, vs, c), v :: v0 :: s) ->
  ...
  | (COp1 (e0, xs, op, vs, c), v :: s) ->
    f e0 xs vs (COp0 (op, vs, c)) (v :: s) t m
...
and f e xs vs c s t m = match e with
  ...
  | Fun (x, e) ->
    begin match (c, s) with
      (CApp0 (vs', c'), VArgs (v1 :: v2s) :: s') -> (* Grab *)
             f e (x :: xs) (v1 :: vs) (CApp0 (vs', c')) (VArgs (v2s) :: s') t m
    | _ -> run_c7 c (VFun (fun c' (v1 :: s') t' m' ->
             f e (x :: xs) (v1 :: vs) c' s' t' m') :: s) t m
    end
  | App (e0, e2s) ->
    f_s e2s xs vs (CApp2 (e0, xs, vs, c)) s t m
...
and f_s e2s xs vs c s t m = match e2s with
  ...
  | e :: e2s ->
    f_s e2s xs vs (CAppS1 (e, xs, vs, c)) s t m
...
and app_s v0 v2s vs c s t m = match v2s with
    [] -> run_c7 c (v0 :: s) t m
  | v1 :: v2s -> app v0 v1 (CApp0 (vs, c)) (VArgs (v2s) :: s) t m
