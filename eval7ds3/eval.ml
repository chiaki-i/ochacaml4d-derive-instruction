...
and run_c7' c' vs c s t m = match (c', s) with
  ...
  | (CApp2 (e0, xs), VArgs (v2s) :: s) ->
    f e0 xs vs (CSeq (CApp0, vs, c)) (VArgs (v2s) :: s) t m
  | (COp0 (op), v :: v0 :: s) ->
  ...
  | (COp1 (e0, xs, op), v :: s) ->
    f e0 xs vs (CSeq (COp0 (op), vs, c)) (v :: s) t m
  | (CAppS0, v :: VArgs (v2s) :: s) ->
    run_c7 c (VArgs (v :: v2s) :: s) t m
  | (CAppS1 (e, xs), VArgs (v2s) :: s) ->
    f e xs vs (CSeq (CAppS0, vs, c)) (VArgs (v2s) :: s) t m
...
and f e xs vs c s t m = match e with
  ...
  | Op (e0, op, e1) -> f e1 xs vs (CSeq (COp1 (e0, xs, op), vs, c)) s t m
  | Fun (x, e) ->
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (CSeq (CApp2 (e0, xs), vs, c)) s t m
...
and f_s e2s xs vs c s t m = match e2s with
  ...
  | e :: e2s ->
    f_s e2s xs vs (CSeq (CAppS1 (e, xs), vs, c)) s t m
...
and app_s v0 v2s vs c s t m = match v2s with
    [] -> run_c7 c (v0 :: s) t m
  | v1 :: v2s -> app v0 v1 (CSeq (CApp0, vs, c)) (VArgs (v2s) :: s) t m
