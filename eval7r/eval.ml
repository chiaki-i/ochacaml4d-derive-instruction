...
let rec f e xs vs c s r t m = match e with
  ...
  | Var (x) -> c (List.nth vs (Env.off_set x xs) :: s) r t m
  | Op (e0, op, e1) ->
    let f_e1_xs = f e1 xs in
    let f_e0_xs = f e0 xs in
    f_e1_xs vs (fun (v1 :: s1) (VEnv (vs) :: r1) t1 m1 ->
        f_e0_xs vs (fun (v0 :: v1 :: s0) r0 t0 m0 ->
            app_op7 op v0 v1 c s0 r0 t0 m0
        ) (v1 :: s1) r1 t1 m1
    ) s (VEnv (vs) :: r) t m
  | Fun (x, e) ->
      ...
      (fun (v1 :: s1) (VEnv (vs) :: r1) t1 m1 ->
          f_e0_xs vs
            (fun (v0 :: VArg (v1) :: s0) r0 t0 m0 ->
                app v0 v1 c s0 r0 t0 m0
            ) (VArg (v1) :: s1) r1 t1 m1
      ) s (VEnv (vs) :: r) t m
  | Shift (x, e) ->
    f e (x :: xs) (VContS (c, s, r, t) :: vs) idc [] [] TNil m
  | Control (x, e) ->
    f e (x :: xs) (VContC (c, s, r, t) :: vs) idc [] [] TNil m
  | Shift0 (x, e) ->
    begin match m with
  ...
  | Reset (e) -> f e xs vs idc [] [] TNil (MCons ((c, s, r, t), m))
...
and app_op7 op v0 v1 c s1 r1 t1 m1 = match (v0, v1) with
    (VNum (n0), VNum (n1)) ->
    begin match op with
