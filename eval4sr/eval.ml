...
and f e xs vs c s r t m = match e with
  ...
  | Var (x) -> run_c4 c (List.nth vs (Env.off_set x xs)) s r t m
  ...
  | Shift (x, e) -> f e (x :: xs) (VContS (c, s, r, t) :: vs) [] [] [] TNil m
  | Control (x, e) -> f e (x :: xs) (VContC (c, s, r, t) :: vs) [] [] [] TNil m
  | Shift0 (x, e) ->
    begin match m with
  ...
  | Reset (e) -> f e xs vs [] [] [] TNil (MCons ((c, s, r, t), m))
