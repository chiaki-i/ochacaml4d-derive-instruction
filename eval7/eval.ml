...
let rec f e xs vs c s t m =
  begin match e with
    ...
    | Op (e0, op, e1) ->
      f e1 xs vs (fun s1 t1 m1 ->
          begin match s1 with
              v1 :: VEnv (vs) :: s1 -> (* VNum を Stack から取り出す *)
              f e0 xs vs (fun s0 t0 m0 ->
                  begin match s0 with
    ...
    | Fun (x, e) ->
      let vfun = VFun (fun vs_out c' s' t' m' -> (* add vs_out *)
          begin match s' with
              v :: s' -> f_t e (x :: xs) (v :: vs) vs_out c' s' t' m'
            | _ -> failwith "stack error"
          end) in
...
and ret c (v :: VEnv (vs_out) :: s) t m = match vs_out with (* expanding CRet (c) *)
    [] -> c (v :: s) t m
  | first :: rest -> app v first rest c s t m

(* f_st: e list -> string list -> v list -> v list -> s -> t -> m *)
and f_t e xs vs vs_out c s t m =
  match e with
        ...
        [] ->
          let vfun = VFun (fun vs_out c' s' t' m' ->
            begin match s' with v :: s' ->
                f_t e (x :: xs) (v :: vs) vs_out c' s' t' m'
              | _ -> failwith "stack error"
            end)
          in c (vfun :: s) t m
      | first :: rest -> f_t e (x :: xs) (first :: vs) rest c s t m
    end
  | App (e0, e1, e2s) ->
    f_st e2s xs vs vs_out (* expanding CApp2 (e0, e1, xs, c) *)
      (fun s2s t2s m2s ->
        begin match s2s with VEnv (v2s) :: VEnv (vs) :: s ->
...
(* f_st: e list -> string list -> v list -> v list -> s -> t -> m *)
and f_st e2s xs vs vs_out cs s t m = match e2s with
    [] -> cs (VEnv (vs_out) :: s) t m (* vs_out might not empty *)
  | first :: rest ->
    f_st rest xs vs vs_out (* expanding CAppS1 (first, xs, c) *)
      (fun s1 t1 m1 ->
        begin match s1 with VEnv (v1) :: VEnv (vs) :: s ->
...
(* app : v -> v -> v list -> c -> s -> t -> m -> v *)
  and app v0 v1 vs_out c s t m = match v0 with
