...
let rec apply9s v0 v2s vs c s t m = match v2s with
    [] -> run_c9 c (v0 :: s) t m
  | v1 :: v2s ->
    apply9 v0 v1 (CSeq ((fun vs c (v :: VArgs (v2s) :: s) t m ->
      apply9s v v2s vs c s t m), vs, c)) (VArgs (v2s) :: s) t m

(* apply : c' *)
let apply = fun vs c s t m -> match s with
  v0 :: VArgs (v2s) :: s -> apply9s v0 v2s vs c s t m

let num n = fun vs c s t m -> run_c9 c (VNum (n) :: s) t m

let access n = fun vs c s t m -> run_c9 c (List.nth vs n :: s) t m

let pushmark = fun vs c s t m -> run_c9 c (VArgs ([]) :: s) t m

let push = fun vs c (v :: VArgs (v2s) :: s) t m -> run_c9 c (VArgs (v :: v2s) :: s) t m
...
let grab i = fun vs c s t m ->
  begin match (c, s) with
    ...
    | _ ->
      run_c9 c (VFun (fun c' (v1 :: s') t' m' -> i (v1 :: vs) c' s' t' m') :: s) t m
    end
...
(*
(* f9 : e -> string list -> v list -> c -> s -> t -> m -> v *)
let rec f9 e xs vs c s t m = match e with
    Num (n) -> run_c9 c (VNum (n) :: s) t m
  | Var (x) -> run_c9 c (List.nth vs (Env.offset x xs) :: s) t m
  | Op (e0, op, e1) ->
    f9 e1 xs vs (CSeq ((fun vs c (v1 :: s) t m ->
      f9 e0 xs vs (CSeq ((fun vs c (v0 :: v1 :: s) t m ->
        begin match (v0, v1) with
            (VNum (n0), VNum (n1)) ->
            begin match op with
                Plus -> run_c9 c (VNum (n0 + n1) :: s) t m
  ...
  | Fun (x, e) ->
    ...
    | _ -> run_c9 c (VFun (fun c' (v1 :: s') t' m' ->
             f9 e (x :: xs) (v1 :: vs) c' s' t' m') :: s) t m
    end
  | App (e0, e2s) ->
    f9s e2s xs vs (CSeq ((fun vs c (VArgs (v2s) :: s) t m ->
      f9 e0 xs vs (CSeq ((fun vs c (v :: VArgs (v2s) :: s) t m ->
        apply9s v v2s vs c s t m), vs, c)) (VArgs (v2s) :: s) t m
    ), vs, c)) s t m
  | Shift (x, e) -> f9 e (x :: xs) (VContS (c, s, t) :: vs) C0 [] TNil m
  | Control (x, e) -> f9 e (x :: xs) (VContC (c, s, t) :: vs) C0 [] TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f9 e (x :: xs) (VContS (c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f9 e (x :: xs) (VContC (c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f9 e xs vs C0 [] TNil (MCons ((c, s, t), m))

(* f9s : e list -> string list -> v list -> c -> s -> t -> m -> v list *)
and f9s e2s xs vs c s t m = match e2s with
    [] -> run_c9 c (VArgs ([]) :: s) t m
  | e :: e2s ->
    f9s e2s xs vs (CSeq ((fun vs c (VArgs (v2s) :: s) t m ->
      f9 e xs vs (CSeq ((fun vs c (v :: VArgs (v2s) :: s) t m ->
