...
let rec run_c4 c v s r t m = match (c, s, r) with
  ...
  | (CApp1 (e0, xs) :: c, s, VEnv (vs) :: r) ->
    f4 e0 xs vs (CApp0 :: c) (VArg (v) :: s) r t m
  | (COp0 (e0, xs, op) :: c, s, VEnv (vs) :: r) ->
    f4 e0 xs vs (COp1 (op) :: c) (v :: s) r t m
  | (COp1 (op) :: c, v0 :: s, r) ->
...
and f4 e xs vs c s r t m = match e with
  ...
  | Op (e0, op, e1) ->
    f4 e1 xs vs (COp0 (e0, xs, op) :: c) s (VEnv (vs) :: r) t m
  | Fun (x, e) ->
  ...
  | App (e0, e1, _) ->
    f4 e1 xs vs (CApp1 (e0, xs) :: c) s (VEnv (vs) :: r) t m
...
let rec run_c4 c v s t m = match (c, s) with
  ...
  | (COp2 (e0, xs, op) :: c, VEnv (vs) :: VArgs (vs_out) :: s) -> (* tail version *)
    f4 e0 xs vs (COp3 (op) :: c) (v :: VArgs (vs_out) :: s) t m
...
and runs_c4 c v s t m = match (c, s) with
    ...
    f4 e1 xs vs (CApp1 (e0, xs) :: c) (VArgs (v) :: VEnv (vs) :: s) t m
