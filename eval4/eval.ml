...
and f4 e xs vs c s t m = match e with
  ...
  | Fun (x, e) -> run_c4 c (VFun (e, x, xs, vs)) s t m
  | App (e0, e1, e2s) ->
...
and f4t e xs vs c s t m = match e with
    Num (n) -> run_c4 (CRet :: c) (VNum (n)) s t m
  | Var (x) -> run_c4 (CRet :: c) (List.nth vs (Env.offset x xs)) s t m
  | Op (e0, op, e1) ->
    f4 e1 xs vs (COp0 (e0, xs, op) :: c) (VEnv (vs) :: s) t m
  | Fun (x, e) -> run_c4 c (VFun (e, x, xs, vs)) s t m
  | App (e0, e1, e2s) ->
    f4st e2s xs vs (CApp2 (e0, e1, xs) :: c) (VEnv (vs) :: s) t m
  | Shift (x, e) -> f4 e (x :: xs) (VContS (c, s, t) :: vs) [] [] TNil m
  | Control (x, e) -> f4 e (x :: xs) (VContC (c, s, t) :: vs) [] [] TNil m
  | Shift0 (x, e) ->
    begin match m with
