...
let rec run_c5 c v s r t m = match (c, s, r) with
  ...
  | (CAppT0 (c), VArgs (v2s) :: s, r) -> apply5st v v2s c s r t m
  | (CAppS0 (cs), VArgs (v2s) :: s, r) ->
    run_c5s cs (v :: v2s) s r t m

(* run_c5s : cs -> v list -> s -> r -> t -> m -> v *)
and run_c5s c v2s s r t m = match (c, s, r) with
    (CApp2 (e0, xs, vs, c), s, r) ->
    f5 e0 xs vs (CApp0 (c)) (VArgs (v2s) :: s) r t m
  | (CAppS1 (e, xs, vs, c), s, r) ->
    f5 e xs vs (CAppS0 (c)) (VArgs (v2s) :: s) r t m
  | (CAppT2 (e0, xs, vs, c), s, r) ->
    f5 e0 xs vs (CAppT0 (c)) (VArgs (v2s) :: s) r t m
...
(* f5s : e list -> string list -> v list -> cs ->
         s -> r -> t -> m -> v list *)
...
(* f5t : e -> string list -> v list -> c -> s -> r -> t -> m -> v *)
and f5t e xs vs c s r t m = match e with
    Num (n) -> run_c5 c (VNum (n)) s r t m
  | Var (x) -> run_c5 c (List.nth vs (Env.offset x xs)) s r t m
  | Op (e0, op, e1) -> f5 e1 xs vs (COp1 (e0, xs, op, vs, c)) s r t m
  | Fun (x, e) ->
  ...
  | App (e0, e2s) ->
    f5st e2s xs vs (CApp2 (e0, xs, vs, c)) s r t m
  | Shift (x, e) -> f5 e (x :: xs) (VContS (c, s, r, t) :: vs) C0 [] [] TNil m
  | Control (x, e) -> f5 e (x :: xs) (VContC (c, s, r, t) :: vs) C0 [] [] TNil m
  | Shift0 (x, e) ->
    begin match m with
