...
let rec run_c5 c v s t m = match (c, s) with
  ...
  | (CApp (v2s, c), s) -> apply2s v v2s c s t m
  | (CApp1 (v1, v2s, c), s) -> apply2 v v1 v2s c s t m
...
and run_c5s c v2s s t m = match (c, s) with
    (CAppS1 (e, xs, vs, c), s) ->
    f2 e xs vs (CAppS0 (v2s, c)) s t m
  | (CAppS2 (e, xs, vs, c), s) ->
    begin match v2s with (v1 :: v2s) ->
        f2 e xs vs (CApp1 (v1, v2s, c)) s t m
...
and f2 e xs vs c s t m = match e with
    Num (n) -> run_c5 c (VNum (n)) s t m
  | Var (x) -> run_c5 c (List.nth vs (Env.offset x xs)) s t m
  | Op (e0, op, e1) ->
    f2 e1 xs vs (COp1 (e0, xs, op, vs, c)) s t m
  | Fun (x, e) ->
    run_c5 c (VFun (fun v1 v2s c' s' t' m' ->
      f2t e (x :: xs) (v1 :: vs) v2s c' s' t' m')) s t m
  | App (e0, e2s) ->
    f2s e2s xs vs (CAppS2 (e0, xs, vs, c)) s t m
  | Shift (x, e) ->
    f2 e (x :: xs) (VContS (c, s, t) :: vs) idc s TNil m
  | Control (x, e) ->
    f2 e (x :: xs) (VContC (c, s, t) :: vs) idc s TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f2 e (x :: xs) (VContS (c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f2 e (x :: xs) (VContC (c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f2 e xs vs idc s TNil (MCons ((c, s, t), m))

(* f2s : e list -> string list -> v list -> cs -> t -> m -> v list *)
and f2s e2s xs vs cs s t m = match e2s with
    [] -> run_c5s cs [] s t m
  | e :: e2s ->
    f2s e2s xs vs (CAppS1 (e, xs, vs, cs)) s t m

(* f2t : e -> string list -> v list -> v list -> c -> t -> m -> v *)
and f2t e xs vs v2s c s t m =
  let app_c = CApp (v2s, c) in
  match e with
    Num (n) -> run_c5 app_c (VNum (n)) s t m
  | Var (x) -> run_c5 app_c (List.nth vs (Env.offset x xs)) s t m
  | Op (e0, op, e1) -> f2 e1 xs vs (COp1 (e0, xs, op, vs, app_c)) s t m
  | Fun (x, e) ->
    begin match v2s with
      [] ->
      run_c5 c (VFun (fun v1 v2s c' s' t' m' ->
        f2t e (x :: xs) (v1 :: vs) v2s c' s' t' m')) s t m
    | v1 :: v2s -> f2t e (x :: xs) (v1 :: vs) v2s c s t m
    end
  | App (e0, e2s) ->
    f2s e2s xs vs (CAppS2 (e0, xs, vs, app_c)) s t m
  | Shift (x, e) -> f2 e (x :: xs) (VContS (app_c, s, t) :: vs) idc s TNil m
  | Control (x, e) -> f2 e (x :: xs) (VContC (app_c, s, t) :: vs) idc s TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f2 e (x :: xs) (VContS (app_c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f2 e (x :: xs) (VContC (app_c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f2 e xs vs idc s TNil (MCons ((app_c, s, t), m))

(* apply2 : v -> v -> v list -> c -> t -> m -> v *)
and apply2 v0 v1 v2s c s t m =
  let app_c = CApp (v2s, c) in
  match v0 with
...
and apply2s v0 v2s c s t m = match v2s with
    [] -> run_c5 c v0 s t m
  | v1 :: v2s -> apply2 v0 v1 v2s c s t m
