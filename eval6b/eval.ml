    ...
    f6s rest xs vs (* expanding CAppS1 (first, xs, c) *)
      (fun v1 s1 t1 m1 ->
          f6 first xs vs (* expanding CAppS0 (cs) *)
            ...
            ) (VEnv (v1)) (VEnv (v1) :: s1) t1 m1
      ) a s t m

and ret c v (VEnv (vs_out) :: s) t m = match vs_out with (* expanding CRet (c) *)
    [] -> c v s t m
  | first :: rest -> apply6 v first rest c s t m

and f6t e xs vs vs_out c a s t m =
  match e with
  ...
  | Fun (x, e) ->
    begin match vs_out with
        [] -> c (VFun (fun v vs_out c' s' t' m' ->
                f6t e (x :: xs) (v :: vs) vs_out c' v s' t' m')) s t m (* adding vs_out, change f5 to f5t *)
      | first :: rest -> f6t e (x :: xs) (first :: vs) rest c a s t m
    end
  | App (e0, e1, e2s) ->
  ...
  | Reset (e) -> f6 e xs vs idc a [] TNil (MCons ((c, s, t), m))
and f6st e2s xs vs vs_out cs a s t m = match e2s with
    [] -> cs vs_out s t m
  | first :: rest ->
