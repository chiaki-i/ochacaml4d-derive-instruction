...
and f5 e xs vs c s r t m = match e with
  ...
  | Op (e0, Plus, e1) ->
    let f_e1_xs = f5 e1 xs in
    let f_e0_xs = f5 e0 xs in
...
and apply_op5 op v0 v1 c s1 r1 t1 m1 = match (v0, v1) with
    (VNum (n0), VNum (n1)) ->
    begin match op with
      Plus -> run_c5 c (VNum (n0 + n1)) s1 r1 t1 m1
    | Minus -> run_c5 c (VNum (n0 - n1)) s1 r1 t1 m1
    | Times -> run_c5 c (VNum (n0 * n1)) s1 r1 t1 m1
    | Divide ->
      if n1 = 0 then failwith "Division by zero"
      else run_c5 c (VNum (n0 / n1)) s1 r1 t1 m1
    end
