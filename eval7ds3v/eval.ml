...
and run_c7' c' rv c s r t m = match (c', s, rv) with
  | (CApp0, v :: VArgs (v2s) :: s, VS (vs)) -> app_s v v2s vs c s r t m
  | (CApp2 (e0, xs), VArgs (v2s) :: s, VS (vs)) ->
    f e0 xs vs (CSeq (CApp0, c)) (VArgs (v2s) :: s) (VS (vs) :: r) t m
  ...
  | (COp1 (e0, xs, op), v :: s, VS (vs)) ->
    f e0 xs vs (CSeq (COp0 (op), c)) (v :: s) (VS (vs) :: r) t m
  | (CAppS0, v :: VArgs (v2s) :: s, VS (vs)) ->
    run_c7 c (VArgs (v :: v2s) :: s) r t m
  | (CAppS1 (e, xs), VArgs (v2s) :: s, VS (vs)) ->
    f e xs vs (CSeq (CAppS0, c)) (VArgs (v2s) :: s) (VS (vs) :: r) t m
...
and f e xs vs c s r t m = match e with
  ...
  | Op (e0, op, e1) ->
    f e1 xs vs (CSeq (COp1 (e0, xs, op), c)) s (VS (vs) :: r) t m
  | Fun (x, e) ->
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (CSeq (CApp2 (e0, xs), c)) s (VS (vs) :: r) t m
...
and f_s e2s xs vs c s r t m = match e2s with
  ...
  | e :: e2s ->
    f_s e2s xs vs (CSeq (CAppS1 (e, xs), c)) s (VS (vs) :: r) t m
...
and app_s v0 v2s vs c s r t m = match v2s with
    [] -> run_c7 c (v0 :: s) r t m
  | v1 :: v2s ->
    app v0 v1 (CSeq (CApp0, c)) (VArgs (v2s) :: s) (VS (vs) :: r) t m
