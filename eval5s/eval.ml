...
let rec run_c5 c v s t m = match (c, s) with
  ...
  | (COp0 (e0, xs, vs, op, c), s) -> f5 e0 xs vs (COp1 (op, c)) (v :: s) t m
  | (COp1 (op, c), v0 :: s) ->
...
and f5 e xs vs c s t m = match e with
  ...
  | Op (e0, op, e1) -> f5 e1 xs vs (COp0 (e0, xs, vs, op, c)) s t m
  | Fun (x, e) ->
    begin match (c, s) with
    (*
      CApp0 (v1, c') -> (* Grab *)
             f5 e (x :: xs) (v1 :: vs) c' t m
             *)
...
and f5s e2s xs vs (cs, c) s t m = match e2s with
    [] -> run_c5s (cs, c) [] s t m
  | e :: e2s ->
    f5s e2s xs vs (CAppS1 (e, xs, vs, cs), c) s t m
