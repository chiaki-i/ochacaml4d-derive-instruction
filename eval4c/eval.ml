...
let rec run_c c v s t m = match (c, s) with
  ...
  | (COp1 (e, xs, op, vs, c), s) -> f e xs vs (COp0 (v, op, c)) v s t m
  | (COp0 (v0, op, c), s) ->
  ...
  | (CApp2 (c), v2s :: s) -> run_cs c v ((v :: v2s) :: s) t m
  | (CApp3 (c), v2s :: s) -> app_s v c (v2s :: s) t m
  | _ -> failwith (s_to_string s)

(* run_cs : c -> s -> t -> m -> v *)
and run_cs c a s t m = match (c, s) with
    (CAppS1 (e, xs, vs, c), v2s :: s) -> f e xs vs (CApp1 (c)) a (v2s :: s) t m
  | (CAppS2 (e, xs, vs, c), v2s :: s) -> f e xs vs (CApp2 (c)) a (v2s :: s) t m
  | (CAppS1T (e, xs, vs, c), v2s :: v2s' :: s) ->
    f e xs vs (CApp1 (c)) a ((v2s @ v2s') :: s) t m

(* f : definitional interpreter *)
(* f : e -> string list -> v list -> c -> a -> s -> t -> m -> v *)
and f e xs vs c a s t m =
  match e with
    Num (n) -> run_c c (VNum (n)) s t m
  | Var (x) -> run_c c (List.nth vs (Env.off_set x xs)) s t m
  | Op (e0, op, e1) -> f e1 xs vs (COp1 (e0, xs, op, vs, c)) a s t m
  | Fun (x, e) ->
    run_c c (VFun (fun v1 c' s' t' m' ->
      f_t e (x :: xs) (v1 :: vs) c' v1 s' t' m')) s t m
  | App (e0, e2s) ->
    f_s e2s xs vs (CAppS1 (e0, xs, vs, c)) a s t m
...
(* f_t : e -> string list -> v list -> v list -> c -> a -> s -> t -> m -> v *)
and f_t e xs vs c a s t m =
  let app_c = CApp3 (c) in
  match e with
    Num (n) -> run_c app_c (VNum (n)) s t m
  | Var (x) -> run_c app_c (List.nth vs (Env.off_set x xs)) s t m
  | Op (e0, op, e1) -> f e1 xs vs (COp1 (e0, xs, op, vs, app_c)) a s t m
  | Fun (x, e) ->
    begin match s with
        [] :: s -> run_c c (VFun (fun v1 c' s' t' m' ->
          f_t e (x :: xs) (v1 :: vs) c' v1 s' t' m')) s t m
      | (v1 :: v2s') :: s -> f_t e (x :: xs) (v1 :: vs) c v1 (v2s' :: s) t m
    end
  | App (e0, e2s) ->
    begin match s with v2s' :: s ->
      f_s e2s xs vs (CAppS1T (e0, xs, vs, c)) a (v2s' :: s) t m
    end
  | Shift (x, e) -> f e (x :: xs) (VContS (app_c, s, t) :: vs) idc a [] TNil m
  | Control (x, e) -> f e (x :: xs) (VContC (app_c, s, t) :: vs) idc a [] TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f e (x :: xs) (VContS (app_c, s, t) :: vs) c0 a s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f e (x :: xs) (VContC (app_c, s, t) :: vs) c0 a s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f e xs vs idc a [] TNil (MCons ((app_c, s, t), m))

(* f_s : e list -> string list -> c -> s -> t -> m -> v list *)
and f_s e2s xs vs c a s t m = match e2s with
    [] -> run_cs c a ([] :: s) t m
  | e :: e2s ->
    f_s e2s xs vs (CAppS2 (e, xs, vs, c)) a s t m
...
and app v0 v1 c s t m =
  ...
  | VContC (c', s', t') ->
    run_c c' v1 s' (apnd t' (cons (fun v t m -> app_s v c s t m) t)) m
...
(* f_init : e -> v *)
let f_init expr = f expr [] [] idc (VNum 0) [] TNil MNil
