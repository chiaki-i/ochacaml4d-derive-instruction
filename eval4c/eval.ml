...
let rec run_c c s t m = match (c, s) with
    (C0, (v :: []) :: s) -> begin match t with
        ...
        begin match m with
            MNil -> v
          | MCons ((c0, s, t), m) -> run_c (CApp3 (c0)) (push v s) t m
        end
      | Trail (h) -> h v TNil m
    end
  | (COp1 (e, xs, op, vs, c), s) -> f e xs vs (COp0 (op, c)) s t m
  | (COp0 (op, c), (v :: v0 :: rest) :: s) ->
  ...
  | (CApp1 (c), (v :: v1 :: v2s) :: s) -> app v v1 c (v2s :: s) t m
  | (CApp2 (c), (v :: v2s) :: s) -> run_cs c ((v :: v2s) :: s) t m
  | (CApp3 (c), (v :: v2s) :: s) -> app_s v c (v2s :: s) t m
...
and f e xs vs c s t m =
  match e with
    Num (n) -> run_c c (push (VNum (n)) s) t m
  | Var (x) -> run_c c (push (List.nth vs (Env.off_set x xs)) s) t m
  | Op (e0, op, e1) -> f e1 xs vs (COp1 (e0, xs, op, vs, c)) s t m
  | Fun (x, e) ->
    run_c c (push (VFun (fun c' ((v1 :: v2s) :: s') t' m' ->
  ...
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f_t e (x :: xs) (VContS (c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f_t e (x :: xs) (VContC (c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f e xs vs idc [[]] TNil (MCons ((c, pushmark s, t), m))

(* f_t : e -> string list -> v list -> c -> s -> t -> m -> v *)
and f_t e xs vs c s t m =
  let app_c = CApp3 (c) in
  match e with
    Num (n) -> run_c app_c (push (VNum (n)) s) t m
  | Var (x) -> run_c app_c (push (List.nth vs (Env.off_set x xs)) s) t m
  | Op (e0, op, e1) -> f e1 xs vs (COp1 (e0, xs, op, vs, app_c)) s t m
  | Fun (x, e) ->
    begin match s with
        [] :: s -> run_c c (push (VFun (fun c' ((v1 :: v2s) :: s') t' m' ->
          f_t e (x :: xs) (v1 :: vs) c' (v2s :: s') t' m')) s) t m
  ...
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f_t e (x :: xs) (VContS (app_c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
          f_t e (x :: xs) (VContC (app_c, s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f e xs vs idc [[]] TNil (MCons ((app_c, pushmark s, t), m))
...
and app v0 v1 c s t m =
  ...
  | VContC (c', s', t') ->
    run_c c' (push v1 s') (apnd t' (cons (fun v t m -> app_s v c s t m) t)) m
