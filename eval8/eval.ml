...
(* 見た目は ZINC instruction、インライン展開して eval6 になれば良い。>> が肝。*)
...
(* ここで、vs_out のことを指す *)
let pushmark i = fun c s t m vs_out ->
  let vs_out = [] in
  c vs_out s t m (* vs_out が空であると決め打ちにする *)

(* f8 : e -> string list -> env -> i *)
let rec f8 e xs vs vs_out = match e with
    Num (n) -> num n
  ...
  | App (e0, e1, e2s) ->
    ...
    pushmark >> (f8s e2s xs vs) >> (f8 e1 xs vs) >> (f8 e0 xs vs) >> call
