...
let rec app_s v0 v2s c s t m = match v2s with
    [] -> c (v0 :: s) t m
  | v1 :: v2s ->
    app v0 v1 (fun (v :: VArgs (v2s) :: s) t m ->
      app_s v v2s c s t m) (VArgs (v2s) :: s) t m

(* app : c' *)
let app = fun c s t m -> match s with
  v0 :: VArgs (v2s) :: s -> app_s v0 v2s c s t m
...
let rec f e xs vs c s t m = match e with
  ...
  | Fun (x, e) ->
    ...
    (*
      (CSeq (i', c'), VArgs (v1 :: v2s) :: s') when i' == app -> (*Grab*)
             f e (x :: xs) (v1 :: vs)
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (fun (VArgs (v2s) :: s) t m ->
      f e0 xs vs (fun (v :: VArgs (v2s) :: s) t m ->
        app_s v v2s c s t m) (VArgs (v2s) :: s) t m
    ) s t m
...
and f_s e2s xs vs c s t m = match e2s with
  ...
  | e :: e2s ->
    f_s e2s xs vs (fun (VArgs (v2s) :: s) t m ->
      f e xs vs (fun (v :: VArgs (v2s) :: s) t m ->
        c (VArgs (v :: v2s) :: s) t m
      ) (VArgs (v2s) :: s) t m
    ) s t m
