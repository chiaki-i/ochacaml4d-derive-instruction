...
let rec run_c7 c s t m = match (c, s) with
  ...
  | (CApp (c), v :: s) -> app_s v c s t m (* return *)
...
and f e xs vs c s t m = match e with
  ...
  | Reset (e) ->
    f e xs vs idc [] TNil (MCons ((c, (VEmpty :: s), t), m))
...
and f_t e xs vs c s t m =
  ...
  | Var (x) -> run_c7 app_c (List.nth vs (Env.off_set x xs) :: s) t m
  ...
  | Shift (x, e) ->
    f e (x :: xs) (VContS (app_c, s, t) :: vs) idc [] TNil m
  | Control (x, e) ->
    f e (x :: xs) (VContC (app_c, s, t) :: vs) idc [] TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
...
and f_sr e xs vs c s t m =
  ...
  | Var (x) -> run_c7 app_c0 (List.nth vs (Env.off_set x xs) :: s) t m
  | Op (e0, op, e1) ->
  ...
  | Reset (e) ->
    f e xs vs idc [] TNil (MCons ((app_c0, (VEmpty :: s), t), m))
