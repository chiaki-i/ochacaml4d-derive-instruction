...
and f7t e xs vs c s t m = match s with VArgs (v2s) :: s ->
  ...
  match e with
    Num (n) -> run_c7 app_c (VNum (n) :: app_s) t m
  | Var (x) -> run_c7 app_c (List.nth vs (Env.offset x xs) :: app_s) t m
  | Op (e0, op, e1) -> f7 e1 xs vs (COp1 (e0, xs, op, vs, app_c)) app_s t m
  | Fun (x, e) ->
  ...
  | Control (x, e) ->
    f7 e (x :: xs) (VContC (app_c, app_s, t) :: vs) idc [] TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f7 e (x :: xs) (VContS (app_c, app_s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f7 e (x :: xs) (VContC (app_c, app_s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f7 e xs vs idc [] TNil (MCons ((app_c, app_s, t), m))

(* apply7 : v -> v -> c -> s -> t -> m -> v *)
and apply7 v0 v1 c (VArgs (v2s) :: s) t m =
  let (app_c, app_s) = (CApp (c), VArgs (v2s) :: s) in
  match v0 with
    VFun (f) -> f c (v1 :: VArgs (v2s) :: s) t m
  | VContS (c', s', t') -> run_c7 c' (v1 :: s') t' (MCons ((app_c, app_s, t), m))
  | VContC (c', s', t') ->
    run_c7 c' (v1 :: s') (apnd t' (cons (fun v t m -> run_c7 app_c (v :: app_s) t m) t)) m
