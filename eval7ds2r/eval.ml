...
let rec run_c7 c s r t m = match (c, s, r) with
  ...
  | (CApp0 (vs, c), v :: VArgs (v2s) :: s, r) -> apply7s v v2s vs c s r t m
  | (CApp2 (e0, xs, vs, c), VArgs (v2s) :: s, r) ->
    f7 e0 xs vs (CApp0 (vs, c)) (VArgs (v2s) :: s) r t m
  | (CAppS0 (vs, c), v :: VArgs (v2s) :: s, r) ->
    run_c7 c (VArgs (v :: v2s) :: s) r t m
  | (CAppS1 (e, xs, vs, c), VArgs (v2s) :: s, r) ->
    f7 e xs vs (CAppS0 (vs, c)) (VArgs (v2s) :: s) r t m
  ...
  | (COp1 (e0, xs, op, vs, c), v :: s, r) ->
    f7 e0 xs vs (COp0 (op, vs, c)) (v :: s) r t m
  | (CRet (vs, C0), v :: s, VK (c') :: r') ->
    run_c7 c' (v :: s) r' t m
...
and f7 e xs vs c s r t m = match e with
  ...
  | Fun (x, e) ->
    ...
    | _ -> run_c7 c (VFun (fun c_ret (v1 :: s') (VK (c') :: r') t' m' ->
             f7 e (x :: xs) (v1 :: vs) c_ret s' (VK (c') :: r') t' m') :: s)
                r t m
    end
  | App (e0, e2s) ->
...
and f7s e2s xs vs c s r t m = match e2s with
  ...
  | e :: e2s ->
    f7s e2s xs vs (CAppS1 (e, xs, vs, c)) s r t m

(* apply7 : v -> v -> c -> s -> r -> t -> m -> v *)
and apply7 v0 v1 c s r t m = match v0 with
    VFun (f) -> f (CRet ([], C0)) (v1 :: s) (VK (c) :: r) t m
  | VContS (c', s', r', t') ->
...
and apply7s v0 v2s vs c s r t m = match v2s with
    [] -> run_c7 c (v0 :: s) r t m
  | v1 :: v2s -> apply7 v0 v1 (CApp0 (vs, c)) (VArgs (v2s) :: s) r t m
