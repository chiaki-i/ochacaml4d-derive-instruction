...
let rec run_c5 c v s t m = match (c, s) with
  ...
  | (CRet :: c, VArgs (v2s) :: s) -> app_s v v2s c s t m
...
and run_c5s c v2s s t m = match (c, s) with
    (CAppT0 (e0, xs, vs) :: c, s) ->
    f_t e0 xs vs v2s c s t m
  | (CAppS1 (e, xs, vs) :: c, s) ->
    f e xs vs (CAppS0 :: c) (VArgs (v2s) :: s) t m
...
and f e xs vs c s t m = match e with
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (CAppT0 (e0, xs, vs) :: c) s t m
...
and f_t e xs vs v2s c s t m =
  ...
  | Var (x) -> run_c5 ret_c (List.nth vs (Env.off_set x xs)) ret_s t m
  | Op (e0, op, e1) -> f e1 xs vs (COp1 (e0, xs, op, vs) :: ret_c) ret_s t m
  | Fun (x, e) ->
    begin match v2s with
      [] ->
      run_c5 c (VFun (fun v1 c' t' m' ->
        f e (x :: xs) (v1 :: vs) c' t' m')) s t m
 (* | v1 :: v2s -> f e (x :: xs) (v1 :: vs)
                      (fun v t m -> app_s v v2s c t m) t m *)
    | v1 :: v2s -> f_t e (x :: xs) (v1 :: vs) v2s c s t m
    end
  | App (e0, e2s) ->
    f_s e2s xs vs (CAppT0 (e0, xs, vs) :: ret_c) ret_s t m
  | Shift (x, e) ->
      f e (x :: xs) (VContS (ret_c, ret_s, t) :: vs) idc [] TNil m
  | Control (x, e) ->
      f e (x :: xs) (VContC (ret_c, ret_s, t) :: vs) idc [] TNil m
  | Shift0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f e (x :: xs) (VContS (ret_c, ret_s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "shift0 is used without enclosing reset"
    end
  | Control0 (x, e) ->
    begin match m with
        MCons ((c0, s0, t0), m0) ->
        f e (x :: xs) (VContC (ret_c, ret_s, t) :: vs) c0 s0 t0 m0
      | _ -> failwith "control0 is used without enclosing reset"
    end
  | Reset (e) -> f e xs vs idc [] TNil (MCons ((ret_c, ret_s, t), m))
...
and app_s v0 v2s c s t m = match v2s with
    [] -> run_c5 c v0 s t m
  | v1 :: v2s -> app v0 v1 (CRet :: c) (VArgs (v2s) :: s) t m
