...
and run_c7' c' vs c s r t m = match (c', s, r) with
  | (CApp0, v :: VArgs (v2s) :: s, r) -> app_s v v2s vs c s r t m
  | (CApp2 (e0, xs), VArgs (v2s) :: s, r) ->
    f e0 xs vs (CSeq (CApp0, vs, c)) (VArgs (v2s) :: s) r t m
  | (COp0 (op), v :: v0 :: s, r) ->
  ...
  | (COp1 (e0, xs, op), v :: s, r) ->
    f e0 xs vs (CSeq (COp0 (op), vs, c)) (v :: s) r t m
  | (CAppS0, v :: VArgs (v2s) :: s, r) ->
    run_c7 c (VArgs (v :: v2s) :: s) r t m
  | (CAppS1 (e, xs), VArgs (v2s) :: s, r) ->
    f e xs vs (CSeq (CAppS0, vs, c)) (VArgs (v2s) :: s) r t m
...
and f e xs vs c s r t m = match e with
  ...
  | Op (e0, op, e1) -> f e1 xs vs (CSeq (COp1 (e0, xs, op), vs, c)) s r t m
  | Fun (x, e) ->
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (CSeq (CApp2 (e0, xs), vs, c)) s r t m
...
and f_s e2s xs vs c s r t m = match e2s with
  ...
  | e :: e2s ->
    f_s e2s xs vs (CSeq (CAppS1 (e, xs), vs, c)) s r t m
...
and app_s v0 v2s vs c s r t m = match v2s with
    [] -> run_c7 c (v0 :: s) r t m
  | v1 :: v2s -> app v0 v1 (CSeq (CApp0, vs, c)) (VArgs (v2s) :: s) r t m
