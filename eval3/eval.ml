...
let rec run_c3 c v t m = match c with
  ...
  | CApp0 (v1) :: c -> app v v1 c t m
  | CApp1 (e0, xs, vs) :: c ->
    f e0 xs vs (CApp0 (v) :: c) t m
  | COp0 (e0, xs, vs, op) :: c -> f e0 xs vs (COp1 (v, op) :: c) t m
  | COp1 (v0, op) :: c ->
...
and f e xs vs c t m = match e with
  ...
  | Op (e0, op, e1) -> f e1 xs vs (COp0 (e0, xs, vs, op) :: c) t m
  | Fun (x, e) ->
    begin match c with
      CApp0 (v1) :: c' -> (* Grab *)
             f e (x :: xs) (v1 :: vs) c' t m
    | _ -> run_c3 c (VFun (fun v1 c' t' m' ->
             f e (x :: xs) (v1 :: vs) c' t' m')) t m
    end
  | App (e0, e1, _) ->
    f e1 xs vs (CApp1 (e0, xs, vs) :: c) t m
