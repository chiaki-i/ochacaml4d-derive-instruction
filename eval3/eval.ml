...
and f3 e xs vs c t m = match e with
  ...
  | Fun (x, e) -> run_c3 c (VFun (e, x, xs, vs)) t m
  | App (e0, e1, e2s) ->
...
and apply3 v0 v1 v2s c t m = match v0 with
    ...
    begin match v2s with
        [] -> f3t e (x :: xs) (v1 :: vs) c t m
      | first :: rest -> f3t e (x :: xs) (v1 :: vs) (CApp0 (first, rest) :: c) t m
    end
...
and f3t e xs vs c t m =
  match e with
    Num (n) -> run_c3 (CRet :: c) (VNum (n)) t m
  | Var (x) -> run_c3 (CRet :: c) (List.nth vs (Env.offset x xs)) t m
  | Op (e0, op, e1) -> f3 e1 xs vs (COp0 (e0, xs, vs, op) :: c) t m
  | Fun (x, e) -> run_c3 c (VFun (e, x, xs, vs)) t m
  | App (e0, e1, e2s) ->
    f3st e2s xs vs (CApp2 (e0, e1, xs, vs) :: c) t m
  | Shift (x, e) -> f3 e (x :: xs) (VContS (c, t) :: vs) [] TNil m
  | Control (x, e) -> f3 e (x :: xs) (VContC (c, t) :: vs) [] TNil m
  | Shift0 (x, e) ->
    begin match m with
  ...
  | Reset (e) -> f3 e xs vs [] TNil (MCons ((c, t), m))
and f3st e2s xs vs cs t m = match e2s with
    [] -> runs_c3 cs [] t m
  | first :: rest ->
    f3st rest xs vs (CAppS1 (first, xs, vs) :: cs) t m
