...
and run_c2s c v2s t m = match c with
    CAppS1 (e, xs, vs, c) -> f e xs vs (CAppS0 (v2s, c)) t m
  | CAppS2 (e, xs, vs, c) ->
    begin match v2s with (v1 :: v2s) ->
        f e xs vs (CApp1 (v1, v2s, c)) t m
...
and f_s e2s xs vs cs t m = match e2s with
    [] -> run_c2s cs [] t m
  | e :: e2s ->
    f_s e2s xs vs (CAppS1 (e, xs, vs, cs)) t m
...
and f_t e xs vs v2s c t m =
  ...
  | Var (x) -> run_c2 app_c (List.nth vs (Env.off_set x xs)) t m
  ...
  | App (e0, e2s) ->
    f_s e2s xs vs (CAppS2 (e0, xs, vs, app_c)) t m
...
and f_sr e xs vs v2s c t m =
  ...
  | Var (x) -> run_c2 app_c0 (List.nth vs (Env.off_set x xs)) t m
  | Op (e0, op, e1) ->
    f e1 xs vs (COp1 (e0, xs, op, vs, app_c0)) t m
  | Fun (x, e) ->
    run_c2 app_c0 (VFun (fun v1 v2s c' t' m' ->
              f_t e (x :: xs) (v1 :: vs) v2s c' t' m')) t m
  | App (e0, e2s) ->
    f_s e2s xs vs (CAppS2 (e0, xs, vs, app_c0)) t m
